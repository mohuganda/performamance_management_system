...
import { formatDate as formatUtilDate, formatTime as formatUtilTime } from '../../utils/date';
...
const BaseAttendanceScreen: React.FC<AttendanceScreenProps> = ({ clocks }) => {
  ...
  // Sort and process attendance logs from local DB (Newest First) — Excludes OOS duty clocks
  const sortedClocks = useMemo(() => {
    return [...clocks]
      .filter((item) => !item.outOfStationRequestId)
      .sort((a, b) => {
  ...
  const formatTime = (timeStr?: string) => {
    if (!timeStr) return '—';
    return formatUtilTime(timeStr);
  };

  const formatDate = (timeStr?: string) => {
    if (!timeStr) return '—';
    return formatUtilDate(timeStr, 'ddd, MMM D, YYYY');
  };
