...
func Api() {
	...
	facades.Route().Prefix("api/v1").Group(func(router route.Router) {
		...
		router.Middleware(authenticate).Group(func(auth route.Router) {
			...
			auth.Get("/auth/totp/status", authController.TotpStatus)
			auth.Post("/auth/totp/enroll", authController.TotpEnroll)
			auth.Post("/auth/totp/confirm", authController.TotpConfirm)
			auth.Post("/auth/totp/disable", authController.TotpDisable)
			...
			auth.Prefix("admin/rbac").Middleware(middleware.Permission("auth.roles.manage")).Group(func(rbac route.Router) {
				...
				rbac.Post("/users/{id}/reset-authenticator", rbacAdminController.ResetUserAuthenticator)
				rbac.Post("/users/{id}/link-staff-by-email", rbacAdminController.LinkUserStaffByEmail)
				rbac.Post("/users/{id}/send-activation", rbacAdminController.SendUserActivation)
			...
			auth.Prefix("admin/staff").Middleware(staffPerm).Group(func(staff route.Router) {
				...
				staff.Post("/{id}/send-activation", staffAdminController.SendActivation)
				staff.Post("/{id}/reset-authenticator", staffAdminController.ResetStaffAuthenticator)
